...
| H12 | Los tests y el arranque del servicio abortan con `KeyError: 'API_KEY'` en una máquina limpia si no existe el archivo `.env` | `config.py` lee `API_KEY` y `CLAVE_FIRMA` con `os.environ[...]`; sin un `.env` creado, `load_dotenv()` no las define y el import de `main` lanza `KeyError` | M2 · 2. El protocolo HTTP y la autenticación | v0-semilla | `pytest tests/test_contrato.py -v` | `KeyError: 'API_KEY'` | Se documenta en el README el paso de crear `.env` (con `API_KEY` y `CLAVE_FIRMA`) antes de instalar y arrancar |
| H13 | Al cargar `modelo.pkl`, scikit-learn emite `InconsistentVersionWarning` | El modelo fue entrenado con `sklearn==1.7.2` pero se carga con `==1.9.0`, la versión fijada en `requirements.txt` | M5 · 8. Resumen y mejores prácticas | commit final (el modelo viaja en el repo tal cual) | `uvicorn main:app --host 0.0.0.0 --port 8000` | `InconsistentVersionWarning: Trying to unpickle estimator Pipeline from version 1.7.2 when using version 1.9.0` | No bloqueante (los 11 tests pasan); se documenta que conviene reentrenar el modelo con la versión fijada |
| H14 | Verificación de arranque limpio y de las correcciones en una máquina con Python 3.12 e instalación limpia de `requirements.txt` | Se reproduce la puesta en marcha desde cero para confirmar el criterio de arranque limpio del servicio | M5 · 8. Resumen y mejores prácticas | commit final | `pytest tests/test_contrato.py -v` y `curl -s -o /dev/null -w "%{http_code}" localhost:8000/health` (también `/siniestros/99999`, `/score` sin poliza, `/exportar`) | `11 passed` / `200` / `404` / `422` / `application/json` | Confirmado: el servicio arranca limpio y reproduce las correcciones documentadas (H3-H7) |
